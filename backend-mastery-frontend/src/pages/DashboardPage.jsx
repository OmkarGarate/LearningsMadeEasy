...
import { CURRICULUM, DAILY_CHALLENGES } from '../data/curriculum';
import ProgressBar from '../components/ProgressBar';
import { Flame, Zap, Trophy, ArrowRight, Target, BookOpen } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 animate-fade-in">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            {nextConcept ? (
              ...
              <Link to="/badges" className="btn-primary">
                You've completed everything! View badges <Trophy size={16} />
              </Link>
            )}
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
        ...
        <StatCard icon={Trophy} label="Badges"    value={badgeCount}             suffix="/ 16" color="from-purple-500 to-pink-500" />
      ...
      <section>
        <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2"><BookOpen size={20} /> Phases</h2>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {CURRICULUM.map(phase => {
            const done = phase.concepts.filter(c => completed[c.id]).length;
            return (
              <Link key={phase.id} to={`/learn/${phase.id}`} className="card glass-hover group">
